...
def generate_attendance_image(header, data_rows, color_mapping, filename="attendance.png"):
    # Image settings
    font_path = "arial.ttf" # Default system font
    try:
        font = ImageFont.truetype(font_path, 18)
        bold_font = ImageFont.truetype(font_path, 18, layout_engine=ImageFont.Layout.RAQM)
    except IOError:
    ...
    total_height = header_height + len(data_rows) * row_height
    ...
    for i, col_name in enumerate(header):
        d.rectangle([x_offset, 0, x_offset + column_widths[i], header_height], fill=(200, 200, 200), outline=(0, 0, 0))
        d.text((x_offset + cell_padding, (header_height - bold_font.getbbox(col_name)[3]) / 2), col_name, fill=(0, 0, 0), font=bold_font)
