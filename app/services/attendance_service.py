...
class AttendanceService:
    ...
    def record_attendance(self, member_id, record_date_str, status_id, notes=None):
        ...
        self.record_repository.add_or_update(member_id, record_date_str, status_id, notes)
    ...
    def bulk_update_status(self, record_ids, new_status_id):
        """Updates the status for multiple attendance records."""
        self.record_repository.bulk_update_status(record_ids, new_status_id)

    def get_attendance_for_date_range_for_image(self, start_date, end_date):
        """
        Generates attendance summary for a given date range, for image generation.
        """
        all_members = self.member_repository.get_all()
        na_status = self.status_repository.get_by_status("NA")
        na_status_id = na_status['id'] if na_status else None
        ...
        header = ["Team Member"] + [day.strftime("%a %b %d") for day in date_range]
        data_rows = []
        for member_name, daily_attendance in attendance_summary.items():
            row = [member_name]
            for day in date_range:
